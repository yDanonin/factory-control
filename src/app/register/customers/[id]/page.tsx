            ...
            >
                    ...
                    <DropdownMenuItem className="cursor-pointer" onSelect={(event) => event.preventDefault()}>
                      <AlertDialog>
                        <Modal
                          ...
                          rowData={customer}
                          idRowData={customer.id}
                          onDelete={() => { router.push('/register/customers') }}
                        />
                      </AlertDialog>
                    </DropdownMenuItem>
                    </DropdownMenuContent>
              ...
              <CardContent className="p-6 text-sm">
                <div className="grid gap-3">
                  ...
                  {customer && <DataList items={[
                    {title: "Identificador", data: customer.id.toString()},
                    {title: "Nome", data: customer.name},
                    {title: "Email", data: customer.email},
                    {title: "Telefone Fixo", data: customer.phone ? VMasker.toPattern(customer.phone, '(99) 9999-9999') : "Não informado"},
                    {title: "Telefone Celular", data: customer.cel_number ? VMasker.toPattern(customer.cel_number, '(99) 99999-9999') : "Não informado"},
                    {title: "CPF", data: customer.cpf ? VMasker.toPattern(customer.cpf, '999.999.999-99') : "Não informado" },
                    {title: "CNPJ", data: customer.cnpj ? VMasker.toPattern(customer.cnpj, '99.999.999/0001-99') : "Não informado"},
                  ]} />}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-3">
                    ...
                    <address className="grid gap-0.5 not-italic text-muted-foreground">
                      ...
                      <span>{customer.address.address_number} {customer.address.public_place}</span>
                      <span>{customer.address.city}, {customer.address.state} {VMasker.toPattern(customer.address.zip_code, '99999-999')}</span>
                    </address>
                  </div>
              ...
              <CardFooter className="flex flex-row items-center border-t bg-muted/50 px-6 py-3">
                <div className="text-xs text-muted-foreground">
                  Última atualização: <time dateTime={new Date(customer.updated_at).toString()}>{new Date(customer.updated_at).toLocaleString()}</time>
                </div>
              </CardFooter>
            </Card>
          </div>}
