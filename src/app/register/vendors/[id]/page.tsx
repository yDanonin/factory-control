            ...
            >
                    ...
                    <DropdownMenuItem className="cursor-pointer" onSelect={(event) => event.preventDefault()}>
                      <AlertDialog>
                        <Modal
                          ...
                          rowData={vendor}
                          idRowData={vendor.id}
                          onDelete={() => { router.push('/register/vendors') }}
                        />
                      </AlertDialog>
                    </DropdownMenuItem>
                    </DropdownMenuContent>
              ...
              <CardContent className="p-6 text-sm">
                <div className="grid gap-3">
                  ...
                  {vendor && <DataList items={[
                    {title: "Identificador", data: vendor.id.toString()},
                    {title: "Nome", data: vendor.name},
                    {title: "Nome da Loja", data: vendor.store_name},
                    {title: "Telefone Fixo", data: vendor.phone ? VMasker.toPattern(vendor.phone, '(99) 9999-9999') : "Não informado"},
                    {title: "Telefone Celular", data: vendor.cel_number ? VMasker.toPattern(vendor.cel_number, '(99) 99999-9999') : "Não informado"},
                    {title: "CNPJ", data: vendor.cnpj ? VMasker.toPattern(vendor.cnpj, '99.999.999/0001-99') : "Não informado"},
                  ]} />}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-3">
                    ...
                    <address className="grid gap-0.5 not-italic text-muted-foreground">
                      ...
                      <span>{vendor.address.address_number} {vendor.address.public_place}</span>
                      <span>{vendor.address.city}, {vendor.address.state} {VMasker.toPattern(vendor.address.zip_code, '99999-999')}</span>
                    </address>
                  </div>
                  <div className="grid auto-rows-max gap-3">
                    <div className="font-semibold">Complemento</div>
                    <div className="text-muted-foreground">
                      {vendor.address.complement}
                    </div>
                  </div>
                </div>
                <Separator className="my-4" />
              ...
              <CardFooter className="flex flex-row items-center border-t bg-muted/50 px-6 py-3">
                <div className="text-xs text-muted-foreground">
                  Última atualização: <time dateTime={new Date(vendor.updated_at).toString()}>{new Date(vendor.updated_at).toLocaleString()}</time>
                </div>
              </CardFooter>
            </Card>
          </div>}
