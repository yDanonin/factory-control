          ...
          {machine && <div>
            ...
            >
                    ...
                    <DropdownMenuItem className="cursor-pointer" onSelect={(event) => event.preventDefault()}>
                      <AlertDialog>
                        <Modal
                          ...
                          rowData={machine}
                          idRowData={machine.id}
                          onDelete={() => { router.push('/register/machines') }}
                        />
                      </AlertDialog>
                    </DropdownMenuItem>
                    </DropdownMenuContent>
              ...
              <CardFooter className="flex flex-row items-center border-t bg-muted/50 px-6 py-3">
                <div className="text-xs text-muted-foreground">
                  Última atualização: <time dateTime={new Date(machine.updated_at).toString()}>{new Date(machine.updated_at).toLocaleString()}</time>
                </div>
              </CardFooter>
            </Card>
          </div>}
